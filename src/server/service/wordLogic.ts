...
async function execSqlForRowsNList(paramObj: any, sqlParam?: any[]) {
    // Usage example:
    let rs: ExecResult = {
        error: null,
        except_case: "",
    ...
    try {
        ...
        const totalResult = (total as AllSettledReturnType).value;
        const listResult = (list as AllSettledReturnType).value;

        console.log(testP.status === 'rejected' ? testP.reason : testP.value);
        console.log(totalResult?.error ? totalResult?.error?.sqlMessage : totalResult);
        console.log(listResult ? listResult : list);


        rs.rows = listResult.data;
        rs.cur_rows = listResult.data.length;
        if (totalResult.data) {
            const _pageSize = paramObj.pageSize ? paramObj.pageSize : rs.cur_rows;
            rs.total_rows = totalResult.data[0].TotalCount;
            rs.max_page = Math.floor(rs.total_rows / _pageSize) + ((rs.total_rows % _pageSize) > 0 ? 1 : 0);
        }
        else {
            rs.except_case = "Catch total_rows failure... ";
            rs.except_case_desc = totalResult?.error?.sqlMessage;
        }
...
const WordLogic = {
    ...
    _add: async function (wordObj: any) {
    ...
    wordList: async function (query: any) {
    ...
    wordDetail: async function (wort: string, id: number) {
    ...
    wordUpdate: async function (wordObj: any) {
    ...
    deleteWord: async function (wordObj: any) {
